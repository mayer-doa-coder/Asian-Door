        ...
        function renderCart(data) {
            ...
            tbody.innerHTML = items.map(function (item) {
                ...
                return [
                    ...
                    '</div>',
                    '</td>',
                    '<td class="fw-semibold">' + formatCurrency(item.unitPrice) + '</td>',
                    '<td><span class="cart-qty-pill">' + Number(item.quantity || 0) + '</span></td>',
                    '<td class="fw-bold text-danger">' + formatCurrency(item.lineTotal) + '</td>',
                    '<td>',
                    '<button type="button" class="btn btn-sm btn-outline-danger js-remove-from-cart" data-product-id="' + item.productId + '">',
